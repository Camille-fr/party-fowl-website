...
*/
// DOM-ready init guard and idempotent flag keep this layout loader stable inside Next App Router refreshes.
(function () {
  'use strict';

  const PF_LAYOUT_INIT_FLAG = '__PF_LAYOUT_INIT__';
  if (window[PF_LAYOUT_INIT_FLAG]) return;
  window[PF_LAYOUT_INIT_FLAG] = true;
  ...
  const LAYOUT_MANAGED_NEXT = 'next';
  const isManagedByNext = (mount) => !!(mount && mount.dataset && mount.dataset.layoutManaged === LAYOUT_MANAGED_NEXT);
  const isNextManagedLayout = () => Boolean(document.querySelector(`[data-layout-managed="${LAYOUT_MANAGED_NEXT}"]`));
  ...
  async function mountPartials() {
    ...
    const headerManaged = isManagedByNext(mounts.head);
    const footerManaged = isManagedByNext(mounts.foot);

    const headerPromise = headerManaged ? Promise.resolve('') : fetchPartial(headerURL);
    const footerPromise = footerManaged ? Promise.resolve('') : fetchPartial(footerURL);

    const [hHTML, fHTML] = await Promise.all([headerPromise, footerPromise]);
    if (!headerManaged) parseAndMount(hHTML, mounts.head, 'header');
    if (!footerManaged) parseAndMount(fHTML, mounts.foot, 'footer');
  ...
  function initNextManagedLayout() {
    watchHeaderResize();
    updateHeaderHeight();
    window.addEventListener('resize', updateHeaderHeight, { passive: true });
    initContactEnhancements();
  ...
  function init() {
    if (isNextManagedLayout()) {
      initNextManagedLayout();
    } else {
      initLegacyLayout();
    }
  }
